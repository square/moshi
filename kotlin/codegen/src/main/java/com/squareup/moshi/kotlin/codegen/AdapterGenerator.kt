...
internal class AdapterGenerator(
  ...
  fun generateFile(messager: Messager, generatedOption: TypeElement?): FileSpec {
    ...
    // jsonAdapter() extension function
    val rawReceiver = when (originalTypeName) {
    ...
    val jsonAdapterFunction = jsonAdapterFunBuilder(
        KClass::class.asClassName().parameterizedBy(originalTypeName))
        .also { jsonAdapterFunBody(it) }
        .build()
    result.addFunction(jsonAdapterFunction)

    // Companion object extension function
    companionObjectName?.let { name ->
      val parametersArray = jsonAdapterFunction.parameters.toTypedArray()
      val parametersByName = parametersArray.joinToString(", ", transform = ParameterSpec::name)
      result.addFunction(jsonAdapterFunBuilder(rawReceiver.nestedClass(name))
          .addAnnotation(AnnotationSpec.builder(Deprecated::class.java)
              .addMember("message = %S", "Replaced by {the class}::class.jsonAdapter(...)")
              .addMember("replaceWith = %T(expression = %S)", ReplaceWith::class.java,
                  "${rawReceiver.canonicalName}::class.jsonAdapter($parametersByName)")
              .addMember("level = %T.WARNING", DeprecationLevel::class.java)
              .build())
          .also { jsonAdapterFunBody(it) }
          .build()
      )
  ...
  private fun jsonAdapterFunBuilder(receiver: TypeName): FunSpec.Builder {
    val builder = FunSpec.builder("jsonAdapter")
        .receiver(receiver)
  ...
  private fun jsonAdapterFunBody(builder: FunSpec.Builder) {
    val adapterTypeVariables = typeVariables
    builder.apply {
      if (adapterTypeVariables.isNotEmpty()) {
        builder.addModifiers(INLINE)
        val reifiedTypeVariables = adapterTypeVariables.map { it.reified(true) }
        val indexedTypeVariableStrings = reifiedTypeVariables.mapIndexed { i, _ -> "%${i + 2}T" }
        val typeVariablesTemplate = indexedTypeVariableStrings.joinToString(", ")
        val typeVariablesClassTemplate = indexedTypeVariableStrings.joinToString(", ") {
          "$it::class.java"
        }
        val moshiParamIndex = 2 + reifiedTypeVariables.size
        addTypeVariables(reifiedTypeVariables)
        addStatement(
            "return %1N<$typeVariablesTemplate>(%${moshiParamIndex}N, arrayOf($typeVariablesClassTemplate))",
            adapterName,
            *reifiedTypeVariables.toTypedArray(),
            moshiParam)
      } else {
        addStatement("return %N(%N)", adapterName, moshiParam)
      }
    }
  }
}
