...
internal class AdapterGenerator(
  ...
  fun generateFile(generatedOption: TypeElement?): (Filer) -> Unit {
    ...
    val (type, bridgeType) = generateType(generatedOption)
    result.addType(type)
    val fileSpec = result.build()
    return { filer ->
      fileSpec.writeTo(filer)
      bridgeType?.let { bridge ->
        val classBytes = bridge.generateClassBytes()
        filer.createClassFile(
            "${bridge.packageName}.${bridge.name}",
            *type.originatingElements.toTypedArray()
        ).apply {
          openOutputStream().use { it.write(classBytes) }
        }
      }
    }
  }

  private fun generateType(generatedOption: TypeElement?): Pair<TypeSpec, BridgeGenerator?> {
    ...
    val (fromJsonFun, bridgeGenerator) = generateFromJsonFun()
    result.addFunction(fromJsonFun)
    result.addFunction(generateToJsonFun())

    return result.build() to bridgeGenerator
  }
  ...
  private fun generateFromJsonFun(): Pair<FunSpec, BridgeGenerator?> {
    ...
    var bridgeGenerator: BridgeGenerator? = null
