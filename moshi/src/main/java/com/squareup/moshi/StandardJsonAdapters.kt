...
internal object StandardJsonAdapters {
  ...
  val FACTORY: JsonAdapter.Factory = object : JsonAdapter.Factory {
    override fun create(
      ...
      if (type == Integer::class.javaPrimitiveType) return INTEGER_JSON_ADAPTER
      ...
      if (type == Integer::class.javaObjectType) return INTEGER_JSON_ADAPTER.nullSafe()
      ...
      if (rawType.isEnum) {
        ...
        return EnumJsonAdapter(rawType as Class<out Enum<*>>).nullSafe()
      }
      return null
    }
  }
  ...
  fun rangeCheckNextInt(reader: JsonReader, typeMessage: String, min: Int, max: Int): Int {
    ...
    if (value < min || value > max) {
      throw JsonDataException(String.format(ERROR_FORMAT, typeMessage, value, reader.getPath()))
    }
    return value
  }
  ...
  internal class EnumJsonAdapter<T : Enum<T>>(private val enumType: Class<T>) : JsonAdapter<T>() {
    ...
    init {
      try {
        ...
        options = JsonReader.Options.of(*nameStrings.filterNotNull().toTypedArray())
