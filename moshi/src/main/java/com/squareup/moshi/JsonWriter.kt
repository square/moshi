...
/**
 * Writes a JSON ([RFC 7159](http://www.ietf.org/rfc/rfc7159.txt)) encoded value to a stream, one
 * token at a time. The stream includes both literal values (strings, numbers, booleans and nulls)
 * as well as the begin and end delimiters of objects and arrays.
 *
 * ## Encoding JSON
 *
 ...
 *   appropriate [value] methods or by nesting other arrays and objects. Finally close the array
 *   using [endArray].
 * * To write **objects**, first call [beginObject]. Write each of the object's properties by
 *   alternating calls to [name] with the property's value. Write property values with the
 *   appropriate [value] method or by nesting other objects or arrays. Finally close the object
 *   using [endObject].
 *
 * ## Example
 *
 ...
 * ```
 *
 * Each `JsonWriter` may be used to write a single JSON stream. Instances of this class are not
 * thread safe. Calls that would result in a malformed JSON string will fail with an
...
public sealed class JsonWriter : Closeable, Flushable {
  ...
  @JvmField protected var scopes: IntArray = IntArray(32)

  @JvmField protected var pathNames: Array<String?> = arrayOfNulls(32)

  @JvmField protected var pathIndices: IntArray = IntArray(32)
  ...
  /**
   * A string containing only whitespace, used for each level of indentation. If empty, the encoded
   * document will be compact.
   */
   ...
   * JSON as specified by [RFC 7159](http://www.ietf.org/rfc/rfc7159.txt). Setting the writer to
   * lenient permits the following:
   ...
   */
  @get:CheckReturnValue public var isLenient: Boolean = false
   ...
   * array elements. The default is false.
   */
  @get:CheckReturnValue public var serializeNulls: Boolean = false

  @JvmField protected var promoteValueToName: Boolean = false
   ...
   *   flattened.
   * - If negative, it is the bitwise inverse (~) of the deepest stack size whose begin/end pairs
   *   have been flattened.
   *
   ...
   * provide a new state. The previous state is returned from [beginFlatten] and restored with
   * [endFlatten].
   */
  @JvmField protected var flattenStackSize: Int = -1
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun beginArray(): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun endArray(): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun beginObject(): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun endObject(): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun name(name: String): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun value(value: String?): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun nullValue(): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun value(value: Boolean): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun value(value: Double): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun value(value: Long): JsonWriter
   ...
   * @return this writer.
   */
  @Throws(IOException::class) public abstract fun value(value: Number?): JsonWriter
   ...
   * encoding. You **must** call [BufferedSink.close] before interacting with this `JsonWriter`
   * instance again.
   *
   * Since no validation is performed, options like [serializeNulls] and other writer configurations
   * are not respected.
   */
  @CheckReturnValue @Throws(IOException::class) public abstract fun valueSink(): BufferedSink
   ...
   *
   * In this example, calling this method allows two sequential calls to [value] to produce the
   * object, `{"a": "b"}`.
   *
   * ```java
  ...
  public fun promoteValueToName() {
    ...
    check(context == NONEMPTY_OBJECT || context == EMPTY_OBJECT) { "Nesting problem." }
    promoteValueToName = true
  }

  /**
   * Cancels immediately-nested calls to [beginArray] or [beginObject] and their matching calls to
   * [endArray] or [endObject]. Use this to compose JSON adapters without nesting.
   *
   ...
   *
   * Emit: `[1, [2, 3, 4], 5]` To produce: `[1, 2, 3, 4, 5]`
   *
   * It also flattens objects within objects. Do not call [name] before writing a flattened object.
   *
   * Emit: `{"a": 1, {"b": 2}, "c": 3}` To Produce: `{"a": 1, "b": 2, "c": 3}`
   *
   * Other combinations are permitted but do not perform flattening. For example, objects inside of
   * arrays are not flattened:
   *
   * Emit: ` [1, {"b": 2}, 3, [4, 5], 6]` To Produce: `[1, {"b": 2}, 3, 4, 5, 6]`
   *
   * This method returns an opaque token. Callers must match all calls to this method with a call to
   * [endFlatten] with the matching token.
   */
  @CheckReturnValue
