...
public abstract class JsonAdapter<T> {
  ...
  /**
   * Returns a JSON adapter equal to this, but that throws a {@link JsonDataException} when {@link
   * JsonReader#setSkipForbidden(boolean) unknown values} are encountered. This constraint applies
  ...
  public final JsonAdapter<T> failOnUnknown() {
    ...
    return new JsonAdapter<T>() {
      @Override public T fromJson(JsonReader reader) throws IOException {
        boolean skipForbidden = reader.isSkipForbidden();
        reader.setSkipForbidden(true);
        try {
          return delegate.fromJson(reader);
        } finally {
          reader.setSkipForbidden(skipForbidden);
        }
      }
      @Override public void toJson(JsonWriter writer, T value) throws IOException {
        delegate.toJson(writer, value);
